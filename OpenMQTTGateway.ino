...
//timer for OnOff
struct timer_on_off_struct {
  int pin;
  long milli;
  long onOffdelay;
  int publishDelay;
};
timer_on_off_struct timer_on_off[32];
...
void loop()
{
  ...
  if (!client.connected()) { // not connected
    ...
    for (int i = 0; i < (sizeof(ACTUATOR_ONOFF_PINS) / sizeof(byte)); i++) {
      timer_on_off_struct* timerOnOff = getStruct(ACTUATOR_ONOFF_PINS[i]);
      
      if ( timerOnOff->onOffdelay > 0 && now - timerOnOff->milli > timerOnOff->onOffdelay ) {
        timerOnOff->onOffdelay  = 0;
        trc(F("MQTTtoONOFF timer done"));
        trc(timerOnOff->pin);
        MQTTtoONOFFSwitch(timerOnOff->pin,false,timerOnOff->publishDelay);
      }
    }
