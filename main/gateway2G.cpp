...
void setup2G() {
  THEENGS_LOG_NOTICE(F("_2G_TX_GPIO: %d " CR), _2G_TX_GPIO);
  THEENGS_LOG_NOTICE(F("_2G_RX_GPIO: %d " CR), _2G_RX_GPIO);
  setupGSM(false);
  THEENGS_LOG_TRACE(F("gateway2G setup done " CR));
}

void setupGSM(bool deleteSMS) {
  THEENGS_LOG_TRACE(F("Init 2G module: %d" CR), _2G_PWR_GPIO);
  delay(1000);
  // Power-cycle the module to reset it.
  A6l.powerCycle(_2G_PWR_GPIO);
  THEENGS_LOG_NOTICE(F("waiting for network connection at bd: %d" CR), _2G_MODULE_BAUDRATE);
  A6l.blockUntilReady(_2G_MODULE_BAUDRATE);
  THEENGS_LOG_NOTICE(F("A6/A7 gsm ready" CR));
  signalStrengthAnalysis();
  delay(1000);
  // deleting all sms
  if (deleteSMS) {
    if (A6l.deleteSMS(1, 4) == A6_OK) {
      THEENGS_LOG_NOTICE(F("delete SMS OK" CR));
    } else {
      THEENGS_LOG_ERROR(F("delete SMS KO" CR));
    }
  }
}
...
void signalStrengthAnalysis() {
  ...
  THEENGS_LOG_TRACE(F("Signal strength: %d" CR), signalStrength);
  if (signalStrength < _2G_MIN_SIGNAL || signalStrength > _2G_MAX_SIGNAL) {
    THEENGS_LOG_TRACE(F("Signal too low restart the module" CR));
...
bool _2GtoX() {
  ...
  for (int i = 0; i < unreadSMSNum; i++) {
    ...
    THEENGS_LOG_TRACE(F("Adv data 2GtoMQTT" CR));
...
void Xto2G(const char* topicOri, const char* datacallback) {
  ...
  if (cmpToMainTopic(topicOri, subjectMQTTto2G)) {
    THEENGS_LOG_TRACE(F("MQTTto2G data analysis" CR));
    // 2G DATA ANALYSIS
    String phone_number = "";
    ...
    if (pos0 != -1) {
      ...
      THEENGS_LOG_NOTICE(F("MQTTto2G phone: %s" CR), (char*)phone_number.c_str());
      THEENGS_LOG_NOTICE(F("MQTTto2G sms: %s" CR), (char*)data.c_str());
      if (A6l.sendSMS(phone_number, data) == A6_OK) {
        THEENGS_LOG_NOTICE(F("SMS OK" CR));
      ...
      THEENGS_LOG_ERROR(F("MQTTto2G Fail reading phone number" CR));
    }
  }
}
#  endif
...
void Xto2G(const char* topicOri, JsonObject& SMSdata) {
  if (cmpToMainTopic(topicOri, subjectMQTTto2G)) {
    ...
    THEENGS_LOG_TRACE(F("MQTTto2G json data analysis" CR));
    if (sms && phone) {
      THEENGS_LOG_NOTICE(F("MQTTto2G phone: %s" CR), phone);
      THEENGS_LOG_NOTICE(F("MQTTto2G sms: %s" CR), sms);
      ...
      THEENGS_LOG_ERROR(F("MQTTto2G failed json read" CR));
    }
  }
}
#  endif
