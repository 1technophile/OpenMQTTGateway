...
static int BH1750_i2c_addr = BH1750_I2C_ADDR;

void setupZsensorBH1750() {
  THEENGS_LOG_NOTICE(F("Setup BH1750 on adress: %H" CR), BH1750_i2c_addr);
  Wire.begin();
...
void MeasureLightIntensity() {
  if (millis() > (timebh1750 + TimeBetweenReadingBH1750)) { //retrieving value of Lux, FtCd and Wattsm2 from BH1750
    THEENGS_LOG_TRACE(F("Creating BH1750 buffer" CR));
    ...
    if (Wire.available() != 2) {
      THEENGS_LOG_ERROR(F("Failed to read from LightSensor BH1750!" CR));
    } else {
      i = Wire.read();
      i <<= 8;
      i |= Wire.read();
      ...
      if (Lux != persistedll || bh1750_always) {
        ...
        THEENGS_LOG_TRACE(F("Same lux don't send it" CR));
      }
      ...
      if (ftcd != persistedlf || bh1750_always) {
        ...
        THEENGS_LOG_TRACE(F("Same ftcd don't send it" CR));
      }
      ...
      if (Wattsm2 != persistedlw || bh1750_always) {
        ...
        THEENGS_LOG_TRACE(F("Same wattsm2 don't send it" CR));
      }
